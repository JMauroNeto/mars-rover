...
describe('Entity - Rover', () => {
  ...
  beforeEach(() => {
    ...
    direction = faker.random.objectElement(Direction) as Direction;
  });
  ...
  it('should give error when direction parameter is incorrect', () => {
    ...
    expect(() => (rover = new Rover(x, y, direction, plateau))).to.throw(
      Error,
      "Rover direction must be 'N', 'S', 'E' or 'W'",
    );
    expect(rover).to.be.eq(undefined);
  });
  ...
  it('should give error when rover x position is greater than plateau size', () => {
    ...
    expect(() => (rover = new Rover(x, y, direction, plateau))).to.throw(
      Error,
      'Rover position must be less than plateau size',
    );
    expect(rover).to.be.eq(undefined);
  });
